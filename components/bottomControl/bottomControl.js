...
Component({
  ...
  pageLifetimes: {
    show: async function () {
      ...
      if (playingMusicList.length != 0 && this.data.playingMusicIndex != -1) {
        musicUrl = await request('/song/url', { id: playingMusicList[this.data.playingMusicIndex].id, br: 320000 });
  ...
  methods: {
    ...
    async handleMusicSwitch(current, type) {
      // console.log(playingMusicList);
      // console.log(current);
      musicUrl = await request('/song/url', { id: playingMusicList[current].id, br: 320000 });
